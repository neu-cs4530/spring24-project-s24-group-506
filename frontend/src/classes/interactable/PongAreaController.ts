...
import { BALL_SIZE, PADDLE_HEIGHT, PADDLE_WIDTH, SCREEN_HEIGHT, SCREEN_WIDTH } from '../PongGame';

export type PongEvents = GameEventTypes & {
  stateUpdated: (state: PongGameState) => void;
};

function createEmptyState(): PongGameState {
    return {
        ...
        ballPosition: { x: SCREEN_WIDTH / 2, y: SCREEN_HEIGHT / 2 },
        ballVelocity: { x: 0, y: 0 },

        leftPaddle: { x: PADDLE_WIDTH / 2 + 1, y: SCREEN_HEIGHT / 2 },
        leftPaddleVelocity: 0,

        rightPaddle: { x: SCREEN_WIDTH - PADDLE_WIDTH / 2 - 1, y: SCREEN_HEIGHT / 2 },
...
export default class PongAreaController extends GameAreaController<
  ...
  PongEvents
> {
  protected _state: PongGameState = createEmptyState();

  get state(): PongGameState {
    return this._state;
  }
  ...
  protected _updateFrom(newModel: GameArea<PongGameState>): void {
    ...
    if (newGame) {
      if (!_.isEqual(newGame.state, this._state)) {
        this._state = newGame.state;
   ...
   */
  public async makeMove(direction: PongDirection): Promise<void> {
    const instanceID = this._instanceID;
